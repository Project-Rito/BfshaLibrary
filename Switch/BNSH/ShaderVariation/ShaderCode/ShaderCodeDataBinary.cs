...
namespace BfshaLibrary
{
    ...
    public class ShaderCodeDataBinary : ShaderCodeData, IResData
    {
        ...
        public List<Stream> BinaryData { get; set; }
        ...
        void IResData.Load(BfshaFileLoader loader)  
        {
            ...
            BinaryData = new List<Stream>();
            for (int i = 0; i < 2; i++) //Fixed with 2 binaries
            {
                loader.Seek((long)codeOffsets[i], System.IO.SeekOrigin.Begin);
                if (i == 0)
                    BinaryData.Add(new SubStream(loader.BaseStream, loader.Position, (int)codeSizes[1]));
                else
                    BinaryData.Add(new SubStream(loader.BaseStream, loader.Position, (int)codeSizes[0]));
            }
        }
