...
namespace BfshaLibrary
{
    ...
    public abstract class ResDict : CollectionBase, IEnumerable<KeyValuePair<string, IResData>>, IResData
    {
        ...
        {
            string[] keys = new string[_nodes.Count - 1];
            for (int i = 0; i < _nodes.Count - 1; i++)
                keys[i] = _nodes[i+1].Key;
            return keys;
        }
        ...
        protected abstract IResData LoadNodeValue(BfshaFileLoader loader);

        // ---- METHODS (PRIVATE) --------------------------------------------------------------------------------------

        private Node ReadNode(BfshaFileLoader loader)
        {
            if (loader.IsSwitch)
            {
                return new Node()
