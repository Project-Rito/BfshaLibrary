...
namespace BfshaLibrary.Core
{
    ...
    : base(stream, Encoding.ASCII, leaveOpen)
        ...
        internal BfshaFileSaver(BfshaFile resFile, string fileName)
            : this(resFile, new FileStream(fileName, FileMode.Create, FileAccess.Write, FileShare.Read), false)
        {
        }

        internal BfshaFileSaver(IResData resData, BfshaFile resFile, string fileName)
    : this(resData, resFile, new FileStream(fileName, FileMode.Create, FileAccess.Write, FileShare.Read), false)
        {
        }

        static internal void ExportSection(string fileName, IResData resData, BfshaFile resFile)
        {
            if (resFile.IsPlatformSwitch)
            {
                using (var saver = new Switch.Core.BfshaFileSwitchSaver(resData, resFile, fileName)) {
                    saver.ExportSection();
                }
            }
            else
            {
                using (var saver = new WiiU.Core.BfshaFileWiiUSaver(resData, resFile, fileName)) {
                    saver.ExportSection();
                }
            }
        }
